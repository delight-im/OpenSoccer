<?php include 'zz1.php'; ?>
<title>Leihgaben | Ballmanager.de</title>
<style type="text/css">
<!--
.jaNein input {
	width: 45px;
...
if ($_SESSION['transferGesperrt'] == TRUE) {
			...
			if (mysql_num_rows($bid2) != 0) {
				...
				if (mysql_affected_rows() != 0) {
					...
					if ($transfers_mit_team[$ac_bieter_id] < 2) {
						...
						if ($ac_praemie > 0) {
							$ac_praemie_str = ' für '.number_format($ac_praemie, 0, ',', '.').' € pro Pflichtspiel';
						}
						else {
							$ac_praemie_str = ' ohne Prämie';
						}
						$formulierung = 'Du hast den Spieler <a href="/spieler.php?id='.$ac_spieler.'">'.$getmanager4.'</a>'.$ac_praemie_str.' an einen anderen Verein verliehen.';
						$sql7 = "INSERT INTO ".$prefix."protokoll (team, text, typ, zeit) VALUES ('".$cookie_team."', '".$formulierung."', 'Transfers', ".time().")";
						$sql8 = mysql_query($sql7);
						$formulierung = 'Du hast den Spieler <a href="/spieler.php?id='.$ac_spieler.'">'.$getmanager4.'</a>'.$ac_praemie_str.' ausgeliehen.';
	...
	if ($an2a > 0) { echo '<p><table class="jaNein"><thead><tr class="odd"><th scope="col">Spieler</th><th scope="col">Team</th><th scope="col">Prämie p.E.</th><th scope="col">Annehmen?</th></tr></thead><tbody>';
		while ($an3 = mysql_fetch_assoc($an2)) {
			if (isset($transfers_mit_team[$an3['ids']])) {
				if ($transfers_mit_team[$an3['ids']] >= 2) {
					...
					$dl2 = mysql_query($dl1);
					$_SESSION['last_leihgaben_anzahl']--;
					echo '<tr><td colspan="4">Dein Co-Trainer hat die Anfrage abgelehnt (2-Transfers-Sperre)</td></tr>';
					continue;
				}
			}
			...
			echo '<td><form action="/leihgaben.php" method="POST" accept-charset="utf-8"><input type="hidden" name="spieler" value="'.$an3['spieler'].'" /><input type="hidden" name="bieter" value="'.$an3['bieter'].'" /><input type="hidden" name="praemie" value="'.$an3['praemie'].'" /><input type="submit" name="aktion" value="Ja"'.noDemoClick($cookie_id).' />&nbsp;<input type="submit" name="aktion" value="Nein"'.noDemoClick($cookie_id).' /></form></td>';
		}
		echo '</tbody></table></p>';
		echo '<p><strong>Hinweis:</strong> Bei mehreren Anfragen für denselben Spieler sind die anfragenden Teams nach Kaderstärke sortiert. Das Angebot des schwächsten Teams steht oben.</p>';
	}
	else {
...
$an2a = mysql_num_rows($an2);
if ($an2a > 0) { echo '<p><table><thead><tr class="odd"><th scope="col">Spieler</th><th scope="col">Team</th><th scope="col">Prämie</th><th scope="col">Aktion</th></tr></thead><tbody>';
    while ($an3 = mysql_fetch_assoc($an2)) {
		...
		echo '<td><form action="/leihgaben.php" method="POST" accept-charset="utf-8"><input type="hidden" name="spieler" value="'.$an3['spieler'].'" /><input type="hidden" name="besitzer" value="'.$an3['besitzer'].'" /><input type="submit" name="aktion" value="Zurückziehen"'.noDemoClick($cookie_id).' /></form></td>';
    }
    echo '</tbody></table></p>';
}
else {
...
?>
<p>Du hast die folgenden Spieler in der aktuellen Saison verliehen. Sie kehren nach Saisonende zu Deinem Verein zurück.</p>
<p><table>
<thead>
<tr class="odd">
<th scope="col" title="Mannschaftsteil">MT</th>
<th scope="col" title="Name des Spielers">Name</th>
<th scope="col" title="Alter">AL</th>
<th scope="col" title="Stärke">ST</th>
<th scope="col" title="Pflichtspiele">PS</th>
<th scope="col" title="Ausleihendes Team">Team</th>
<th scope="col" title="Prämie pro Pflichtspiel">Prämie p.P.</th>
</tr>
</thead>
<tbody>
<?php
...
<p><strong>Überschriften:</strong> MT: Mannschaftsteil, AL: Alter, ST: Stärke, PS: Pflichtspiele, p.P.: pro Pflichtspiel</p>
<p><strong>Mannschaftsteile:</strong> T: Torwart, A: Abwehr, M: Mittelfeld, S: Sturm
<?php } else { echo '<p>Du hast in dieser Saison noch keine Spieler verliehen.'; } ?>
</p>
<h1>Ausgeliehene Spieler</h1>
<?php
...
if (mysql_num_rows($sql2) > 0) {
?>
<p>Du hast die folgenden Spieler in der aktuellen Saison von einem anderen Verein ausgeliehen. Sie verlassen Dein Team nach Saisonende wieder.</p>
<p><table>
<thead>
<tr class="odd">
<th scope="col" title="Mannschaftsteil">MT</th>
...
<th scope="col" title="Stärke">ST</th>
<th scope="col" title="Pflichtspiele">PS</th>
<th scope="col" title="Verleihendes Team">Team</th>
<th scope="col" title="Prämie pro Pflichtspiel">Prämie p.P.</th>
</tr>
</thead>
<tbody>
...
<p><strong>Überschriften:</strong> MT: Mannschaftsteil, AL: Alter, ST: Stärke, PS: Pflichtspiele, TO: Tore, p.P.: pro Pflichtspiel</p>
<p><strong>Mannschaftsteile:</strong> T: Torwart, A: Abwehr, M: Mittelfeld, S: Sturm
<?php } else { echo '<p>Du hast in dieser Saison noch keine Spieler ausgeliehen.'; } ?>
</p>
<?php } else { ?>
<h1>Leihgaben</h1>
