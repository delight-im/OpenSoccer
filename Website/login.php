...
if ((isset($_POST['lusername']) && isset($_POST['lpassword'])) OR $valid_bigpoint_user != '') {
    ...
    if ($lologin3 == 1) {
		...
		if (substr($lologin4['username'], 0, 9) == 'GELOESCHT') {
			...
			if (isset($_COOKIE['uniqueHash'])) { $loginLog_uniqueHash = mysql_real_escape_string(trim(strip_tags($_COOKIE['uniqueHash']))); } else { $loginLog_uniqueHash = $loginLog_ip; setcookie('uniqueHash', $loginLog_ip, getTimestamp('+30 days'), '/', '.ballmanager.de', FALSE, TRUE); }
