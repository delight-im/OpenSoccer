...
div.matchReport {
	...
	height: 36px;
	margin-top: 1em;
	background: transparent url(http://s3.amazonaws.com/ballmanager.de/images/match_score_back.png) scroll repeat-x left center;
}
...
div.matchReport div.scores span.score {
	...
	line-height: 36px;
	margin: 0 4px;
	background: transparent url(http://s3.amazonaws.com/ballmanager.de/images/match_score_highlight.png) scroll repeat-x left center;
	text-align: center;
...
if ($ber2a > 0) {
    ...
    while ($ber3 = mysql_fetch_assoc($ber2)) {
		...
		if (mb_substr($kommentar_ergebnisStr, 0, 9, 'UTF-8') == 'Noten für' OR mb_substr($kommentar_ergebnisStr, 0, 23, 'UTF-8') == '<strong>Die Zeitschrift') {
			...
			$kommentar_ergebnisStr = str_replace('</strong>', '</strong> <img src="http://s3.amazonaws.com/ballmanager.de/images/ball_14.png" alt="Tor!" width="14" />', $kommentar_ergebnisStr);
		}
